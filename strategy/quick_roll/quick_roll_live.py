...
from redis_signal_sender import (
    RedisSignalSender,
    get_today_trades,
    ...
    if g.is_live_trading:
        ...
        for position in context.portfolio.positions.values():
            ...
            if avg_cost > 0 and current_price / avg_cost < 0.9:
                log.info(
                    f"止损触发：{position.security} 当前价{current_price:.2f}  成本{avg_cost:.2f}"
                )
    ...
    if g.is_live_trading:
        ...
        if yesterday_positions:
            ...
            for stock_code, position_data in yesterday_positions.items():
                ...
                log.info(
                    f"同步持仓: {stock_code}, 数量: {position_data['volume']}, 成本: {position_data['avg_cost']:.2f}"
                )
...
def filter_limitup_stock(context, stocks):
    ...
    if g.is_live_trading:
        ...
        return [
            s for s in stocks if s in context.portfolio.positions or last[s][-1] < cd[s].high_limit
        ]
...
def filter_limitdown_stock(context, stocks):
    ...
    if g.is_live_trading:
        ...
        return [
            s for s in stocks if s in context.portfolio.positions or last[s][-1] > cd[s].low_limit
        ]
...
def filter_kcbj_stock(stocks):
    return [
        s
        for s in stocks
        if not (s.startswith(("4", "8")) or s.startswith("68") or s.startswith("300"))
    ]
...
def open_position(context, security, value):
    ...
    if g.is_live_trading:
        ...
        signal_id = send_order_signal(
            ...
            extra={"target_value": value, "strategy": "牛市板块快速轮动"},
        )
...
def update_from_redis(context):
        ...
        if signal_id:
            if signal_id in trades_by_signal:
                ...
                if trade.get("direction") == "BUY":
                    ...
                    updated_count += 1
                    log.info(
                        f"确认买入成交: {stock_code}, 数量: {position['volume']}, 价格: {position['avg_cost']:.2f}"
                    )
    ...
    summary = redis_sender.get_today_summary("quick_roll")
    log.info(f"今日成交汇总: 买入{summary['buy_trades']}笔, 卖出{summary['sell_trades']}笔")
    log.info(
        f"买入金额: {summary['total_buy_amount']:.0f}, 卖出金额: {summary['total_sell_amount']:.0f}"
    )
...
def print_position_info(context):
    ...
    if g.is_live_trading:
        ...
        for stock_code, position in g.virtual_positions.items():
            ...
            log.info(
                f"{stock_code} {status} 成本{avg_cost:.2f} 现价{current_price:.2f} "
        ...
        for p in context.portfolio.positions.values():
            ...
            print(
                f"{p.security} 成本{p.avg_cost:.2f} 现价{p.price:.2f} "
