...
from src.daily_pnl_calculator import calculate_daily_summary, DailyPnLCalculator
...
def test_mock_data_summary():
    ...
    try:
        ...
        # 买入订单
        mock_orders.append(type('MockOrder', (), {
        ...
        for stock in summary['stock_breakdown']:
            print(f"• {stock['stock_display']}: ¥{stock['total_amount']:,.2f}, 净量: {stock['net_volume']}")
        
        # 验证时间段分布
        print(f"\n时间段分布:")
        print(f"• 上午: {summary['time_breakdown']['morning']['orders_count']}笔, ¥{summary['time_breakdown']['morning']['amount']:,.2f}")
        print(f"• 下午: {summary['time_breakdown']['afternoon']['orders_count']}笔, ¥{summary['time_breakdown']['afternoon']['amount']:,.2f}")
