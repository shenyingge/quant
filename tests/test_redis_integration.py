...
def test_trade_record_operations():
    ...
    try:
        ...
        success = redis_trade_client.save_trade_record(test_order_id, test_trade_id, test_trade_data)
        if success:
        ...
        if retrieved_record:
            logger.info(f"✓ 交易记录读取成功: {json.dumps(retrieved_record, indent=2, ensure_ascii=False)}")
        else:
    ...
    try:
        ...
        if settings.redis_trade_records_enabled:
            ...
            if redis_connected:
                ...
                logger.info(f"Redis交易记录信息: {json.dumps(redis_info, indent=2, ensure_ascii=False)}")
...
def test_daily_cleanup_scheduler():
    ...
    try:
        ...
        for i in range(5):
            ...
            redis_trade_client.save_trade_record(f'CLEANUP_TEST_{i:03d}', f'CLEANUP_TRADE_{i:03d}', test_data)
