...
def stress_test_concurrent_orders(order_count: int = 50):
    ...
    try:
        ...
        logger.info(
            f"批量提交完成，耗时: {submit_time:.3f}秒, 速度: {order_count/submit_time:.1f} 订单/秒"
        )
        ...
        for i in range(max_wait_time):
            ...
            if current_count != last_count:
                ...
                logger.info(
                    f"[{i+1}s] 进度: {current_count}/{order_count} ({progress:.1f}%), "
...
def stress_test_burst_orders():
    ...
    try:
        ...
        for i in range(90):  # 最多等待90秒
            ...
            if i % 5 == 0:  # 每5秒报告一次
                logger.info(
                    f"[{i}s] 完成: {current_count}/{total_orders}, "
                    f"队列: {queue_status['thread_queue_pending']}"
                )
        ...
        logger.info(
            f"总订单: {total_orders}, 成功: {success_count}, 成功率: {success_count/total_orders*100:.1f}%"
        )
