...
def test_concurrent_orders():
    ...
    try:
        ...
        def order_callback(order_id, error, signal_info):
            with results_lock:
                if order_id:
                    results.append(('success', signal_info['stock_code'], signal_info['direction'], order_id))
                    logger.info(f"✓ {signal_info['stock_code']} {signal_info['direction']} 委托成功: {order_id}")
                else:
                    results.append(('failed', signal_info['stock_code'], signal_info['direction'], error))
                    logger.error(f"✗ {signal_info['stock_code']} {signal_info['direction']} 委托失败: {error}")
        ...
        for i, signal in enumerate(test_signals):
            ...
            logger.info(f"[{i+1}/{len(test_signals)}] 已提交: {signal['stock_code']} {signal['direction']}")
        ...
        for i in range(60):  # 监控60秒
            ...
            if completed != last_completed:
                logger.info(f"[{i+1}s] 已完成: {completed}/{len(test_signals)}, "
        ...
        if pending_orders:
            ...
            for order in pending_orders:
                logger.info(f"  订单 {order['order_id']}: {order['stock_code']} {order['direction']} "
...
def test_rapid_signal_processing():
    ...
    # 模拟快速接收多个交易信号
    signals = [
        {'signal_id': f'SIG_{i:03d}', 'stock_code': f'{1000+i:06d}.SZ', 'direction': 'BUY', 'volume': 100*(i+1), 'price': 1.0 + i*0.1}
        for i in range(10)
    ]
