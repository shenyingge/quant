...
def test_sync_sqlite_to_meta_db_returns_sync_summary(monkeypatch):
    ...
    monkeypatch.setattr("src.trading_meta_sync._read_source_rows", lambda: source_rows)

    async def fake_sync(rows_by_table):
        from src.trading_meta_sync import TradingMetaSyncResult
    ...
    monkeypatch.setattr("src.trading_meta_sync._sync_rows_to_meta_db", fake_sync)
