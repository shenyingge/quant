...
class TradingCalendarManager:
    ...
    def update_calendar_for_year(self, year: int, force: bool = False) -> bool:
        ...
        try:
            # 检查是否已有该年数据
            existing_count = session.query(TradingCalendar).filter(
                TradingCalendar.year == year
            ).count()
            
            if existing_count > 0 and not force:
                logger.info(f"{year}年交易日历已存在，共{existing_count}条记录")
                # 加载到内存缓存
                trading_dates = session.query(TradingCalendar.date).filter(
                    TradingCalendar.year == year,
            ...
            if existing_count > 0 and force:
                # 删除旧数据
                session.query(TradingCalendar).filter(
                    TradingCalendar.year == year
                ).delete()
                session.commit()
            ...
            logger.info(f"成功更新{year}年交易日历：总共{total_days}天，其中{trading_days_count}个交易日")
    ...
    def is_trading_day(self, check_date: Optional[date] = None) -> bool:
        ...
        try:
            # 查询数据库
            calendar_entry = session.query(TradingCalendar).filter(
                TradingCalendar.date == check_date
            ).first()
    ...
    def get_next_trading_day(self, from_date: Optional[date] = None) -> Optional[date]:
        ...
        try:
            # 查询下一个交易日
            next_trading = session.query(TradingCalendar).filter(
                TradingCalendar.date > from_date,
                TradingCalendar.is_trading == True
            ).order_by(TradingCalendar.date).first()
    ...
    def get_previous_trading_day(self, from_date: Optional[date] = None) -> Optional[date]:
        ...
        try:
            # 查询上一个交易日
            prev_trading = session.query(TradingCalendar).filter(
                TradingCalendar.date < from_date,
                TradingCalendar.is_trading == True
            ).order_by(TradingCalendar.date.desc()).first()
