...
from src.database import SessionLocal, OrderRecord, TradingSignal
...
class DailyPnLCalculator:
    ...
    def _calculate_trading_summary(self, orders: List[OrderRecord], target_date: date) -> Dict[str, Any]:
        """计算交易汇总数据"""
    ...
    def _calculate_time_stats(self, orders: List[OrderRecord]) -> Dict[str, Any]:
        ...
        return {
            'morning': {
                ...
                'amount': round(sum(float(o.filled_volume * o.filled_price) for o in morning_orders), 2),
                'volume': sum(o.filled_volume for o in morning_orders)
            },
            'afternoon': {
                'orders_count': len(afternoon_orders),
                'amount': round(sum(float(o.filled_volume * o.filled_price) for o in afternoon_orders), 2),
                'volume': sum(o.filled_volume for o in afternoon_orders)
            }
        }
    
    def _estimate_performance(self, buy_orders: List[OrderRecord], sell_orders: List[OrderRecord]) -> Dict[str, Any]:
        ...
        # 估算交易成本 (假设手续费率0.03%)
        total_amount = sum(float(o.filled_volume * o.filled_price) for o in buy_orders + sell_orders)
        ...
        for code, trades in stock_trades.items():
            if trades['buys'] and trades['sells']:
                # 简单按FIFO匹配
                avg_buy_price = sum(float(o.filled_price * o.filled_volume) for o in trades['buys']) / sum(o.filled_volume for o in trades['buys'])
                avg_sell_price = sum(float(o.filled_price * o.filled_volume) for o in trades['sells']) / sum(o.filled_volume for o in trades['sells'])
