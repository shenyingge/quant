...
from src.trading.trading_calendar_manager import initialize_trading_calendar, trading_calendar_manager
...
class TradingEngine:
    ...
    def start(self):
        ...
        # 发送服务启动通知
        self.notifier.notify_service_status("已启动", "交易引擎成功启动")
    ...
    def stop(self):
        ...
        # 发送服务停止通知
        self.notifier.notify_service_status("已停止", "交易引擎已安全停止")
        logger.info("交易引擎已停止")
                ...
                def check_and_update_next_year():
                    while self.is_running:
                        try:
                            ...
                            # 检查是否已有下一年数据
                            db = SessionLocal()
                            from src.infrastructure.db import TradingCalendar
    ...
    def _run_daily_export(self):
        ...
        try:
            ...
            from src.data_manager.daily_exporter import export_daily_data
